...
void vector_swap(Vector *v, int i, int j) {
  if (i < 0 || i >= v->size || j < 0 || j >= v->size) {
    printf("Error: vector_swap: invalid indices %d and %d for vector with size "
           "%d.\n",
           i, j, v->size);
    exit(0);
  }
...
bool vector_it_has_next(VectorIterator *it) { return it->i < it->v->size; }

void vector_it_destroy(VectorIterator *it) { free(it); }